...
summary(hum.mod)
    ...
    #### Predict ####
## Need to decide on either raster:: predict funciton or stats::predict functions
#devtools::install_github("ropenscilabs/skimr")
library(skimr)

## how different are the x and y coords betweeen the rasters and the ppp
c.id <- unique(humW.df$cell)
xy.ppp <- humW.df %>%
  group_by(cell) %>%
  filter(cell %in% c.id) %>%
  mutate(x_diff = x_grid - x, 
         y_diff = y_grid - y)
skim(xy.ppp$x_diff) ## They appear to be quite different...
skim(xy.ppp$y_diff)


## fuck it predict and remap anyways
...
anOB.w <- pppWeights(ob.col = OB_ann0_,
                        dataFrame = dat.2,
                        rasterGrid = rf)
    #### Covariates ####
an.CoV <- c( c( "ptr_BR", "mic_BR", "mol_BR",
                "ptr_BR_1", "mic_BR_1", "mol_BR_1",
                "ptr_BR_3", "mic_BR_3", "mol_BR_3",
                "ptr_BR_5", "mic_BR_5", "mol_BR_5",
                "logPop", "NB_lDiv","fragIndex",
                "month", "OB_hum0_",  "x", "y", "OB_ann0_"))

    #### Weighted dataframe/ weights vector ####
anW.df <- weightedDf(ob.col = OB_ann0_,
                          obWeights.df = anOB.w,
                          dataFrame = dat.2,
                          cols = an.CoV)
anW.v <- anW.df$.mpl.W
    ...
    #### Model ####
an.form <- as.formula(paste(".mpl.Y ","~", paste(an.CoV[1:(length(an.CoV)-5)],collapse = "+")))
